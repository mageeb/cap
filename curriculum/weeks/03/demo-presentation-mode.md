# Demo B — Excalidraw Frame Presentation Mode
...
Use existing frames as ordered slides: start, fit the current frame into the viewport, navigate previous/next, show `current / total`, and exit to the prior editor state. Use scene element order for frames; keep navigation within bounds and prevent accidental editing during presentation. Preserve Zen/View Mode and persisted scene data. This fork-only prototype excludes slide editing, animations, export, remote presenting, collaboration sync, and changes to Excalidraw+ functionality.
...
```bash
git switch -c feature/frame-presentation-mode
git branch --show-current
gh repo set-default YOUR_USERNAME/excalidraw
```

`git switch -c` creates and switches to the branch. **You should see:** `feature/frame-presentation-mode`. If it already exists from this same exercise, continue it with `git switch feature/frame-presentation-mode` instead of recreating it.
...
```text
We are building a Frame Presentation Mode prototype in this Excalidraw fork.
Use scene element order for frames. Start on frame 1, fit it to the viewport,
navigate previous/next within bounds, show current/total, and exit restoring prior
editor state. Prevent accidental scene edits while presenting. Provide visible
controls and keyboard navigation/exit. Preserve Zen/View Mode, persisted scene
data, and existing Excalidraw+ functionality. Slide editing, animations, export,
remote presenting, and collaboration sync are out of scope.

Use built-in file-reading and search tools to inspect frame, viewport, state,
and UI patterns. Compare 2-3 approaches by reuse, complexity, UX/keyboard risks,
affected files, and tests. Cite files/symbols, recommend one, and wait for our
choice. Handle zero/one/multiple frames safely. Do not edit repository files.
```
...
and edge cases. Cover zero/one/multiple frames; scene element order; starting
on frame 1; fitting the current frame; previous/next within bounds; current/total;
visible controls and exact navigation/exit keys; preventing accidental scene
edits; and restoring the prior viewport and editor modes on exit. Preserve
Zen/View Mode, persisted scene data, and existing Excalidraw+ functionality.
Write only the spec, then show it and stop for our review. Do not implement.
```

**You should see:** Specific actions and expected results you can try in the browser. Agree on the entry control, navigation/exit keys, and state restored on exit before continuing.
...
frame ordering, viewport fitting, view/edit restrictions, navigation/status UI,
...
1. Try starting with no frames; confirm a safe disabled/no-op result.
2. Create one frame using Excalidraw's Frame tool and put a shape/text inside it. Start presenting; verify `1 / 1`, navigation bounds, and exit.
3. Create 3–4 visibly different frames, labeled in creation order. Without reordering them, verify the presentation follows that sequence; left-to-right position on the canvas does not define slide order.
4. Start again; confirm frame 1 appears, fits the viewport, and shows the correct counter.
5. Navigate both directions with the visible controls and agreed keys. Try beyond the first and last frames; confirm navigation stays within bounds.
6. Try editing while presenting. Exit and verify the prior viewport/editor modes are restored and normal editing works.
7. Try Zen Mode and View Mode before and after presenting; confirm their existing behavior and unchanged scene data.
...
**You should see:** `feature/frame-presentation-mode` and the expected feature files. If `git diff` opens a pager, press `q` to return to the prompt.
...
```bash
git commit -m "feat: add frame presentation mode prototype"
```

**You should see:** A new commit ID and a summary of changed files. Then push:

```bash
git push -u origin feature/frame-presentation-mode
```
...
```bash
gh pr create --repo YOUR_USERNAME/excalidraw --base master --head feature/frame-presentation-mode --title "Demo: add frame presentation mode prototype" --body-file docs/ai/PR_BODY.md
```
